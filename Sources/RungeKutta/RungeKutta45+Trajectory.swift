...
extension RungeKutta45 {
    ...
    fileprivate static func stepSizeFactor(errorNorm: Double, tolerance: Double) -> Double {
...
// MARK: - `[Double]`-specialised overloads (Apple Accelerate fast path)
//
// Picked by Swift's overload resolution at the call site when `State` is
// concretely `[Double]`. No runtime type check; no protocol bloat; the
// generic version above remains the implementation for every other
// `NormedVectorState` conformer. Compile-time conditional means non-Apple
// builds (or `-D SWIFTCALX_NO_ACCELERATE`) fall through to the generic
// trajectory transparently.
...
extension RungeKutta45 {
    public static func trajectory(
        at outputTimes: [Double],
        from initialState: [Double],
        derivative: @escaping @Sendable (Double, [Double]) -> [Double],
        startingAt t0: Double = 0,
        tolerance: Double = 1e-6,
        initialStep: Double? = nil,
        minStep: Double? = nil,
    ...
    public static func denseSegments(
        from initialState: [Double],
        derivative: @escaping @Sendable (Double, [Double]) -> [Double],
        startingAt t0: Double = 0,
        through t1: Double,
        ...
        while t < t1, iterations < maxIterations {
            iterations += 1
            let hStep = min(h, t1 - t)
            let attempt = stepDouble(from: t, y: y, k1: k1, size: hStep, derivative: derivative)
