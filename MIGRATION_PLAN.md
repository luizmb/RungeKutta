...
- **Algebraic structure bundle (2026-05-21)**. Follows FP's newtype-wrapper convention from `NumericMonoid<T>.{Sum, Product, Min, Max}` (each newtype conforms directly to `Monoid` / `Semigroup`; fold with `mconcat(_:)` / `sconcat(_:_:)`):
  - `BidimensionalPoint<T>.Additive` / `TridimensionalPoint<T>.Additive` — `Monoid` newtypes wrapping the point under elementwise addition with the origin as identity.
  - `Matrix<Scalar>.Additive` / `Matrix<Scalar>.Multiplicative` — `Semigroup` (not `Monoid`) newtypes; the identities (zero matrix / `Iₙ`) need a runtime shape that a `static var Self` can't carry. Folds with `sconcat(_:_:)` (non-empty input) instead of `mconcat(_:)`.
  - `DerivativeMethod<Scalar>.Composition` — `Monoid` newtype; combine is left-to-right composition via `then(_:)`, identity is the no-op deriver (order 0).
  - `DerivativeMethod.identity` / `then(_:)` are also surfaced directly on the type — same primitives the newtype wraps, useful when monoidal folding isn't needed.
