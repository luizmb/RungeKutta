...
- **`Vector` opt-in fast state type (2026-05-21)**. Replaces the earlier `[Double]`-specialised trajectory overloads with a cleaner generalisation: a new `Vector` struct (concrete `[Double]`-backed) that conforms to ``VectorState`` and ``NormedVectorState`` with vDSP-backed `+`, `-`, scalar `*`, and `infinityNorm` on Apple (scalar Swift fallback elsewhere). Because the conformance is on a concrete type, the protocol witness for `+` / `*` IS the optimised path — generic solver code (RK45, RK4, Birchall's iterated action, anything else over VectorState) automatically picks up vDSP whenever the state is `Vector`. No specialisation overloads anywhere in the solvers, no runtime type checks, no `.+` operator gymnastics. Consumers opt in at the entry point: `Vector([1.0, 2.0])` or `[1.0, 2.0].asVector` (zero-copy wrapping). `Matrix.apply(to: Vector) -> Vector` bridges so derivative functions stay in `Vector`-land. The earlier `RungeKutta45+DoubleStep.swift` / trajectory overloads are removed — Vector subsumes that work with broader reach.
  - **Other Array-shaped APIs lifted to Vector**: `RandomAccessCollection` (subscript, count, map / filter / reduce), `Equatable` / `Hashable` / `CustomStringConvertible`, `mapVector` (type-preserving map), FP-style `fmap` / `bind` / `kleisli` / `liftA2` / `apply` mirroring `CoreFP/Array+{Functor,Monad,Applicative}`, plus `⋅` operators in `MathOperators` (`Matrix ⋅ Vector`, `Scalar ⋅ Vector`, `Vector ⋅ Vector` dot product backed by `cblas_ddot` on Apple).
